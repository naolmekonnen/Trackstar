...
import {
  Settings, Share2, MapPin, Flame, Trophy, Zap,
  Calendar, TrendingUp, Award, Watch, Users, ChevronRight
} from "lucide-react";
import { currentUser, recentRuns, weeklyStats, formatDuration } from "@/lib/mock-data";
    ...
    <div className="px-5 pt-14 pb-4 max-w-lg mx-auto">
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <Link href="/(app)/settings" className="w-9 h-9 rounded-full bg-white/[0.05] flex items-center justify-center">
        ...
        <div className="relative overflow-hidden rounded-2xl bg-white/[0.03] border border-white/[0.06] p-6 mb-6">
          ...
          <div className="flex gap-3 mt-5">
            <Link href="/(app)/persona" className="flex-1 p-3 rounded-xl bg-white/[0.04] border border-white/[0.06]">
            ...
            <Link href="/(app)/aura" className="flex-1 p-3 rounded-xl bg-gradient-to-r from-[#BFFF00]/8 to-[#00E5FF]/5 border border-white/[0.06]">
              <div className="flex items-center gap-2">
        ...
        <div className="p-4 rounded-xl bg-white/[0.03] border border-white/[0.06] mb-6">
          ...
          <div className="flex items-center justify-between">
            ...
            <Link href="/(app)/predictions" className="text-xs text-[#BFFF00]">
              Prediction →
            </Link>
          </div>
        </div>
        ...
        <div className="mb-6">
          <div className="flex items-center justify-between mb-3">
            ...
            <Link href="/(app)/recap" className="text-xs text-[#BFFF00]">View all</Link>
          </div>
          <div className="flex gap-3 overflow-x-auto hide-scrollbar">
            {recentRuns.map(run => (
              <Link key={run.id} href="/(app)/recap" className="shrink-0 w-32 rounded-xl bg-gradient-to-b from-white/[0.04] to-white/[0.02] border border-white/[0.06] p-3 hover:border-white/[0.1] transition-all">
        ...
        <div className="p-4 rounded-xl bg-white/[0.03] border border-white/[0.06]">
          ...
          {currentUser.clubs.map(club => (
            <Link key={club} href="/(app)/discover" className="flex items-center justify-between py-2 group">

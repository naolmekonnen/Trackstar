...
import { currentUser, getDaysUntilRace, recentRuns, coachMessages, runClubs, weeklyStats } from "@/lib/mock-data";
...
export default function HomePage() {
  ...
  return (
    <div className="px-5 pt-14 pb-4 max-w-lg mx-auto">
      ...
      {/* Today's Workout */}
      <motion.div {...fadeUp(1)}>
        <Link href="/run" className="block p-5 rounded-2xl bg-white/[0.03] border border-white/[0.06] mb-4 group">
      ...
      <motion.div {...fadeUp(2)} className="grid grid-cols-3 gap-3 mb-4">
        <Link href="/plan" className="p-4 rounded-xl bg-white/[0.03] border border-white/[0.06]">
        ...
        <Link href="/profile" className="p-4 rounded-xl bg-white/[0.03] border border-white/[0.06]">
        ...
        <Link href="/predictions" className="p-4 rounded-xl bg-gradient-to-br from-[#BFFF00]/8 to-[#00E5FF]/5 border border-white/[0.06]">
      ...
      {/* Runner Persona Card */}
      <motion.div {...fadeUp(4)}>
        <Link href="/persona" className="block p-4 rounded-xl bg-white/[0.03] border border-white/[0.06] mb-4 group">
      ...
      {/* Latest Run Recap */}
      <motion.div {...fadeUp(5)}>
        <Link href="/recap" className="block p-4 rounded-xl bg-white/[0.03] border border-white/[0.06] mb-4 group">
          <div className="flex items-center justify-between mb-3">
      ...
      {/* Runner Aura */}
      <motion.div {...fadeUp(6)}>
        <Link href="/aura" className="block p-5 rounded-xl relative overflow-hidden mb-4 group">
      ...
      {/* Upcoming Club Run */}
      <motion.div {...fadeUp(7)}>
        <Link href="/discover" className="block p-4 rounded-xl bg-white/[0.03] border border-white/[0.06] group">
      ...
      <motion.div {...fadeUp(8)} className="mt-4 p-4 rounded-xl bg-white/[0.03] border border-white/[0.06]">
        <div className="flex items-center justify-between mb-3">
          <span className="text-xs text-white/40 uppercase tracking-wider">Weekly Volume</span>
          <span className="text-xs text-white/30">{weeklyStats[weeklyStats.length - 1].miles} mi this week</span>
        </div>
      ...
      {/* Motivation */}
      <motion.div {...fadeUp(9)} className="mt-6 text-center">
